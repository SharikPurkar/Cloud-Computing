...
public class SearchEngineTester {
	...
	public static void searchKeyword(String keyword) throws Exception {
	    ...
	    byte[] pageDocBytes=Bytes.toBytes(pageDocId);
	    Get uriIndex=new Get(pageDocBytes);
            //uriIndex.add(Constants.CF_DETAILS_BYTES, Constants.QUAL_URI_BYTES, uriBytes);
	    Result uriRow=dataTable.get(uriIndex);
	    byte[] uriByteArray=uriRow.getValue(Constants.CF_DETAILS_BYTES, Constants.QUAL_URI_BYTES);
	    pageUri=Bytes.toString(uriByteArray);

	    Get pageIndex=new Get(pageDocBytes);
            //uriIndex.add(Constants.CF_DETAILS_BYTES, Constants.QUAL_URI_BYTES, uriBytes);
	    pageIndex.addFamily(Constants.CF_PAGERANK_BYTES);
	    Result pageRow=prTable.get(pageIndex);
	    KeyValue pageKeyValue=pageRow.list().get(0);
	    pageRank=Bytes.toFloat(pageKeyValue.getQualifier());
